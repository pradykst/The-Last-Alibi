import { exploreRequestSchema } from '@alibi/protocol';
...
export function POST(request: Request, context: RouteContext): Promise<Response> {
  return gameRouteResponse(async () => {
    ...
    const body = await parseRequestBody(request, exploreRequestSchema);
    return runFixtureGameOperation(() => fixtureGameService.explore(sessionId, body));
  });
}
