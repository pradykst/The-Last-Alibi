import { accusationRequestSchema } from '@alibi/protocol';
...
export function POST(request: Request, context: RouteContext): Promise<Response> {
  return gameRouteResponse(async () => {
    ...
    const body = await parseRequestBody(request, accusationRequestSchema);
    return runFixtureGameOperation(() => fixtureGameService.accuse(sessionId, body));
  });
}
